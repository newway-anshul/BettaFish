"""
Restitch and render an HTML report using the latest chapter JSON.
"""
...
def render_html(document_ir, base_name, timestamp, ir_path=None):
    """
    Render Document IR to HTML with HTMLRenderer and save it.

    Writes output to `final_reports/html` and logs chart validation stats
    so chart data repair/failure results are easy to inspect.
...
def main():
    """
    Main entry: load latest chapters, stitch IR, and render HTML.
    ...
    Flow:
        ...
        4) Render HTML and print output paths and stats.

    Returns:
        int: 0 means success, non-zero means failure.
    """
    logger.info("🚀 Restitching and rendering HTML with the latest LLM chapters")
    ...
    logger.info("")
    logger.info("🎉 HTML stitching and rendering completed")
