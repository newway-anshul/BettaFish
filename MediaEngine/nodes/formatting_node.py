...
Responsible for formatting final research output into a polished Markdown report
"""
...
class ReportFormattingNode(BaseNode):
    """Node that formats the final report."""
    ...
    def run(self, input_data: Any, **kwargs) -> str:
        """
        Call the LLM to generate a Markdown-formatted report
        
        Args:
            input_data: List containing all paragraph data
        ...
        try:
            if not self.validate_input(input_data):
                raise ValueError("Invalid input format. Expected a list containing title and paragraph_latest_state fields")
            ...
            # Call LLM to generate Markdown output (streaming with safe UTF-8 assembly)
            ...
            logger.info("Formatted report generated successfully")
    ...
    def process_output(self, output: str) -> str:
        ...
        try:
            ...
            # Ensure the report has a basic structure
            if not cleaned_output.strip():
                return "# Report Generation Failed\n\nUnable to generate valid report content."
            
            # Add a default title if one is missing
        ...
        try:
            ...
            if len(paragraphs_data) > 1:
                report_lines.extend([
                    "## Conclusion",
                    "",
                    "This report provides a comprehensive analysis of the topic through deep search and research."
                    "The content above offers important reference points for understanding the subject.",
                    ""
                ])
