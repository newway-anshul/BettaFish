...
class FirstSummaryNode(StateMutationNode):
    ...
    def run(self, input_data: Any, **kwargs) -> str:
        """
        Call the LLM to generate a paragraph summary
        ...
        try:
            ...
            # Read the latest HOST speech if available
            ...
            # If HOST speech exists, prepend it as reference
            ...
            logger.info("Generating initial paragraph summary")
            
            # Call LLM to generate summary (streaming with safe UTF-8 assembly)
            ...
            logger.info("Initial paragraph summary generated successfully")
            return processed_response
            
        except Exception as e:
            logger.exception(f"Failed to generate initial summary: {str(e)}")
            raise e
            ...
            try:
                ...
                if fixed_json:
                    try:
                        ...
                        # If output is not valid JSON, return cleaned text directly
    ...
    def mutate_state(self, input_data: Any, state: State, paragraph_index: int, **kwargs) -> State:
        ...
        try:
            ...
            if 0 <= paragraph_index < len(state.paragraphs):
                ...
                logger.info(f"Updated initial summary for paragraph {paragraph_index}")
            else:
...
class ReflectionSummaryNode(StateMutationNode):
    """Node that updates paragraph summary from reflection search results."""
    ...
    def mutate_state(self, input_data: Any, state: State, paragraph_index: int, **kwargs) -> State:
        """
        Write the updated summary into state
