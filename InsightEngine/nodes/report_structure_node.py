...
Generates the overall report structure based on a query.
"""
...
class ReportStructureNode(StateMutationNode):
    """Node that generates report structure."""
    ...
    def run(self, input_data: Any = None, **kwargs) -> List[Dict[str, str]]:
        """
        Call the LLM to generate a report structure.
        
        Args:
            input_data: Input data (unused; uses query from initialization)
            **kwargs: Extra parameters
            
        Returns:
            List of report structure items
        """
        try:
    ...
    def process_output(self, output: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if not isinstance(report_structure, list):
                logger.info("Report structure is not a list, trying to convert...")
            ...
            for i, paragraph in enumerate(report_structure):
                if not isinstance(paragraph, dict):
                    logger.warning(f"Paragraph {i+1} is not a dictionary, skipping")
                    continue
            ...
            if not validated_structure:
                logger.warning("No valid paragraph structures found, using default structure")
                return self._generate_default_structure()
    ...
    def _generate_default_structure(self) -> List[Dict[str, str]]:
        ...
        return [
            ...
            },
            {
                ...
                "content": "Analyze different aspects of the query topic in depth"
            }
        ]
    ...
    def mutate_state(self, input_data: Any = None, state: State = None, **kwargs) -> State:
        ...
        Args:
            ...
            state: Current state, creates a new one when None
            **kwargs: Extra parameters
