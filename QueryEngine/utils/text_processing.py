...
Used to clean LLM output and parse JSON
"""
...
def clean_json_tags(text: str) -> str:
    """
    Remove JSON code block tags from text
...
def clean_markdown_tags(text: str) -> str:
    """
    Remove Markdown code block tags from text
...
def remove_reasoning_from_output(text: str) -> str:
    ...
    patterns = [
        r'(?:reasoning|thinking|analysis)[:：]\s*.*?(?=\{|\[)',  # Remove reasoning part
        r'(?:explanation|description)[:：]\s*.*?(?=\{|\[)',      # Remove explanation part
...
def fix_incomplete_json(text: str) -> str:
    ...
    Returns:
        Repaired JSON text; returns empty string if unrecoverable
    """
    ...
    # Check if opening array bracket might be missing
    ...
    # Check if ending array bracket might be missing
    ...
    # Repair mismatched brackets
    if open_braces > close_braces:
        text += '}' * (open_braces - close_braces)
    if open_brackets > close_brackets:
        text += ']' * (open_brackets - close_brackets)
    
    # Validate whether repaired JSON is valid
    try:
        json.loads(text)
        return text
    except JSONDecodeError:
...
def fix_aggressive_json(text: str) -> str:
    ...
    if len(objects) >= 2:
        # If there are multiple objects, wrap as array
        return '[' + ','.join(objects) + ']'
    elif len(objects) == 1:
        # If there is only one object, wrap as array
        return '[' + objects[0] + ']'
    else:
...
def update_state_with_search_results(search_results: List[Dict[str, Any]], 
    ...
    if 0 <= paragraph_index < len(state.paragraphs):
        # Get the latest search query (assuming it is the current query)
        current_query = ""
        if search_results:
            # Infer query from search results (this should be improved to get the actual query)
